...
// RegisterPublicRoutes exposes the /api/r/:slug/restaurant subset + a
// standalone /api/restaurants/lookup and signup.
...
func (ctl *RestaurantController) Lookup(c *fiber.Ctx) error {
	slug := c.Query("slug")
	if slug == "" {
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"error": "slug is required"})
	}
	r, err := ctl.svc.GetBySlug(c.UserContext(), slug)
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"error": err.Error()})
	}
	if r == nil {
